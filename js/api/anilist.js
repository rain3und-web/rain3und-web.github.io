...
window.fetchCharacterData = async function(anilistId) {
    ...
    try {
        ...
        let characterList = edges.map(edge => ({
            id: edge.node.id, name: edge.node.name.native || edge.node.name.full, img: edge.node.image?.large || ''
        }));
        document.getElementById('editCharacters').value = JSON.stringify(characterList);
        if(typeof window.renderCharacterList === 'function') window.renderCharacterList();
    } catch (e) { console.error("キャラ自動補完エラー", e); }
};
...
window.translateSynopsis = async function(englishText, animeTitle) {
    ...
    const gasUrl = 'https://script.google.com/macros/s/AKfycbz5tA_NaxljnTEX-_iAIy2mQLPn_HBrUYiR6hHDCLQJlFF01FuXBrssHlX06U09XC9o/exec'; 
    ...
    try {
        ...
        const res = await fetch(gasUrl, {
            method: 'POST',
            // ★ text と一緒に title も送る！
            body: JSON.stringify({ text: cleanText, title: animeTitle }) 
        });
